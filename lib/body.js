...
class File {
  ...
  constructor(fileName, info, fileStream) {
    ...
    fileStream.on("close", () => {
      let final_path = "";
      const ext = mime.extension(this.info.mimeType);
      if (ext != "") {
        final_path = this.tempFolder + "/" + this.tempName + "." + ext;
      } else {
        final_path = this.tempFolder + "/" + this.tempName;
      }

      fs.writeFileSync(final_path, Buffer.concat(dataChunks));
    });
  }
}
